...
func (qq *QuestionQuery) QueryOptions() *OptQuery {
	query := &OptQuery{config: qq.config}
...
func (qq *QuestionQuery) WithOptions(opts ...func(*OptQuery)) *QuestionQuery {
	query := &OptQuery{config: qq.config}
	for _, opt := range opts {
		opt(query)
	}
	qq.withOptions = query
	return qq
}
...
func (qq *QuestionQuery) sqlAll(ctx context.Context) ([]*Question, error) {
	...
	if query := qq.withOptions; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[uuid.UUID]*Question, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Options = []*Opt{}
		}
		var (
			edgeids []uuid.UUID
			edges   = make(map[uuid.UUID][]*Question)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(question.OptionsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, qq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "options": %w`, err)
		}
		query.Where(opt.IDIn(edgeids...))
