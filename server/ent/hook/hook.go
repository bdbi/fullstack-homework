...
// The OptFunc type is an adapter to allow the use of ordinary
// function as Opt mutator.
type OptFunc func(context.Context, *ent.OptMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OptFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.OptMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OptMutation", m)
	}
	return f(ctx, mv)
}
